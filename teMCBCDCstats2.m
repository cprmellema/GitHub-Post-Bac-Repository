...
figure
nmc=size(teMC(rot4),1);
boxplot([teMC2(rot4)-teMC(rot4), teBC(rot4)-teMC(rot4), teDC(rot4)-teMC(rot4)], [ones(1,nmc),2*ones(1,nmc),3*ones(1,nmc)])
saveplot(gcf, './worksheets/2015_11_03-bcireportplots/tecondcomprotated.eps' , 'eps')

teMCMC2 = abs(teMC2-teMC);
teMCBC = abs(teBC-teMC);  
teMCDC = abs(teDC-teMC);

[h1, p1] = ttest2(teMCMC2, teMCBC)
[h2, p2] = ttest2(teMCMC2, teMCDC)

%Compute mean, std

figure
bar([mean(teMCMC2), mean(teMCBC), mean(teMCDC)])
hold on 
errorbar([mean(teMCMC2), mean(teMCBC),mean(teMCDC)], [std(teMCMC2), std(teMCBC),std(teMCDC)])
saveplot(gcf, './worksheets/2015_11_03-bcireportplots/tebarchanges.eps' , 'eps')
saveplot(gcf, './worksheets/2015_11_03-bcireportplots/tebarchanges.jpg' , 'jpg')
