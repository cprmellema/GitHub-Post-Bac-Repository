function [ output ] = GLMlassotest( spikes,distrib,link,backstep,forwardstep)
%
%This function takes data on neural spiking and 
%generates a lassoed generalized linear model with the link function
...
%this finds an appropriate value of lambda for the model
% 
% for k=1:n
k=1;
     spikes2=spikesprime;
...
%         [~,prelambdas{1,k}]= lassoglm(spikes2,spikes0,distrib,'link',link,'CV',3);
         [~,prelambdas]= lassoglm(spikes2,spikes0,distrib,'link',link,'CV',3);
% end


% for z=1:n
%     lambdas{1,z}=prelambdas{1,z}.LambdaMinDeviance;
% end
% 
% lambmins=zeros(1,n);
% 
% for p=1:n
%     lambmins(1,p)=min(lambdas{1,p});
% end
% 
% lambavg = mean(lambmins);
prelambdas=prelambdas.LambdaMinDeviance;
output.lambda = min(prelambdas);


% output.lambda = lambavg;
...
for k=1:n
    ...
    spikes0=spikes(:,k);
        ...
        [coeff{1,k},fitinfo{1,k}]= lassoglm(spikes2,spikes0,distrib,'link',link,'Lambda',output.lambda);
       ...
       spikes0=spikes(:,k);
        ...
        if j~=k
             ...
             elseif j==n
              ...
              [~,fitinfo1]= lassoglm(spikes2,spikes0,distrib,'link',link,'Lambda',output.lambda);
              Deviance(j,k)=fitinfo1.Deviance;
...
%     
%     muhat(:,j)=glmval(coeff(:,j),spikesprime2,link);
% end
% 
% output.predicted=muhat;
% 
